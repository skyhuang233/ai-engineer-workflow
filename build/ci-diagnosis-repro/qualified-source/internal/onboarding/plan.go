...
func Plan(ctx context.Context, options PlanOptions) (setupcontract.Plan, error) {
	...
	snapshotJSON, err := captureApprovalSnapshot(ctx, discovery, repositoryID, zeroBaseline, zeroBaseline)
	if err != nil {
		return setupcontract.Plan{}, err
	}
	...
	requiredChecks := CanonicalRequiredChecks(policy.RequiredChecks)
