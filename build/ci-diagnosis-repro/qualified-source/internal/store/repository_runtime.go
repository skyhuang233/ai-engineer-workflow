...
func recordRepositoryRuntimeConfiguration(ctx context.Context, executor repositoryRecordExecutor, value RepositoryRuntimeConfiguration, conflictAction string) error {
	...
	_, err := executor.ExecContext(ctx, `INSERT INTO repository_runtime_configurations(repository,default_branch,source_path,root_issue_number,workspace_root,state_root,codex_auth_file,github_api_url,poll_interval_seconds,workspace_retention_seconds,max_parallel_runs,updated_at)
