...
try {
  ...
  $resolved = & $resolver -CandidateDirectory $candidateDirectory -ExpectedVersion '0.0.1' -ExpectedSourceCommit $sourceCommit -ExpectedQualificationRunID 42 | ConvertFrom-Json
  ...
  try { & $resolver -CandidateDirectory $candidateDirectory -ExpectedVersion '0.0.1' -ExpectedSourceCommit ('b' * 40) -ExpectedQualificationRunID 42 *> $null } catch { $acceptedWrongSource = $false }
  if ($acceptedWrongSource) { throw 'Candidate resolver accepted the wrong source commit' }

  $acceptedWrongRun = $true
  try { & $resolver -CandidateDirectory $candidateDirectory -ExpectedVersion '0.0.1' -ExpectedSourceCommit $sourceCommit -ExpectedQualificationRunID 43 *> $null } catch { $acceptedWrongRun = $false }
  ...
  try { & $resolver -CandidateDirectory $candidateDirectory -ExpectedVersion '0.0.1' -ExpectedSourceCommit $sourceCommit -ExpectedQualificationRunID 42 *> $null } catch { $acceptedWrongWorker = $false }
  ...
  $acceptedTamper = $true
  try { & $resolver -CandidateDirectory $candidateDirectory -ExpectedVersion '0.0.1' -ExpectedSourceCommit $sourceCommit -ExpectedQualificationRunID 42 *> $null } catch { $acceptedTamper = $false }
