...
Run every operation from the admitted checkout through `workflow github`; never use an independently authenticated `gh` process. The command opens Workflow Home read-only, derives the canonical repository from `origin`, requires an eligible Repository Admission, checks the plaintext PAT fingerprint, live-verifies its personal or organization-owner identity and scopes, and verifies owner-guarded repository access. It never prints or passes the PAT in argv. A personal repository requires `repo,workflow`; an organization repository requires `repo,workflow,admin:org`, active organization-admin membership, and working SSO authorization.
