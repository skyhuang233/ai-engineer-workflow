...
func TestRepositoryRunnerComposesGatewayAndBusinessLoopWithoutPAT(t *testing.T) {
	...
	config := store.RepositoryRuntimeConfiguration{
		...
		WorkspaceRoot: filepath.Join(layout.Workspaces, "owner-repo"), StateRoot: filepath.Join(layout.State, "codex", "owner-repo"),
	...
	for _, expected := range []string{"poll-github", config.Repository, strconv.FormatInt(config.RootIssueNumber, 10), config.SourcePath, config.WorkspaceRoot, config.StateRoot, "http://host.docker.internal:18787", "http://127.0.0.1:18787", config.PollInterval.String()} {
		if !strings.Contains(joinedPoll, expected) {
			t.Fatalf("business-loop arguments lack %q: %q", expected, poll)
		}
	}
	if strings.Contains(joinedPoll, "codex-auth-file") {
		t.Fatal("repository runtime still passes a repository-scoped Codex auth source")
	}
