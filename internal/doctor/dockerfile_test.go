...
func TestWorkerDockerfilePinsAPTInputsAndNoMistakesCommit(t *testing.T) {
	...
	for _, required := range []string{
		...
		fmt.Sprintf("ARG NO_MISTAKES_REPOSITORY=%s", config.NoMistakes.Repository),
		fmt.Sprintf("ARG NO_MISTAKES_COMMIT=%s", config.NoMistakes.Commit),
		...
		"io.workflow.no-mistakes.repository",
		"io.workflow.no-mistakes.commit",
