...
func TestUnifiedPublisherConsumesExactRegistryPreservedQualification(t *testing.T) {
	...
	for _, required := range []string{
		...
		`candidate_source_commit: ${{ steps.candidate.outputs.source_commit }}`,
		`qualification_run_id: ${{ steps.candidate.outputs.qualification_run_id }}`,
		`qualification_run_attempt: ${{ steps.candidate.outputs.qualification_run_attempt }}`,
