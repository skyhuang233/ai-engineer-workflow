...
func (d Dispatcher) Claim(ctx context.Context, repository string, rootNumber, ticketID int64, owner string) (store.TicketClaim, error) {
	...
	body, err := plan.RenderProjection(snapshot.Root.Body, projection)
	if err != nil {
		return claim, err
	}
	if err := d.Projector.UpdateIssueBody(ctx, repository, rootNumber, body); err != nil {
...
func (d Dispatcher) Recover(ctx context.Context, repository string, rootNumber int64) error {
	...
	return d.Projector.UpdateIssueBody(ctx, repository, rootNumber, body)
}
