...
func TestRecoverIsolatesPreparedDeliveryBeforeTerminalization(t *testing.T) {
	ctx := context.Background()
	snapshot := plan.Snapshot{Repository: "owner/repo", Root: plan.Issue{ID: 100, Number: 10, Body: "spec", Labels: []string{plan.PlanLabel}}, Children: []plan.Issue{{ID: 1, Number: 11, Title: "first", Labels: []string{plan.TicketLabel}, State: "open"}}}
	db, err := store.Open(ctx, filepath.Join(t.TempDir(), "workflow.db"))
	if err != nil {
	...
	activeAtIsolation := false
	var isolated []string
	dispatcher := scheduler.Dispatcher{Store: db, Reader: &reader{snapshot: snapshot}, Projector: &projector{}, Recovery: recoveryInspector{containerRunning: false, workspaceReady: true, isolate: func(runID string) {
		isolated = append(isolated, runID)
		runs, queryErr := db.ExpiredLaunchedRecoveryRuns(ctx, version.ID, now)
		activeAtIsolation = queryErr == nil && len(runs) == 1 && runs[0].Claim.RunID == deliveryClaim.RunID
	...
	if len(isolated) != 1 || isolated[0] != deliveryClaim.RunID || !activeAtIsolation {
		t.Fatalf("prepared delivery isolation = runs %#v active=%t", isolated, activeAtIsolation)
	}
	projection, err := db.PlanProjectionAt(ctx, version.ID, now)
	if err != nil {
		t.Fatal(err)
...
func TestRecoverReleasesUnreservedDeliveryWithoutIsolation(t *testing.T) {
	ctx := context.Background()
	snapshot := plan.Snapshot{Repository: "owner/repo", Root: plan.Issue{ID: 100, Number: 10, Body: "spec", Labels: []string{plan.PlanLabel}}, Children: []plan.Issue{{ID: 1, Number: 11, Title: "first", Labels: []string{plan.TicketLabel}, State: "open"}}}
	db, err := store.Open(ctx, filepath.Join(t.TempDir(), "workflow.db"))
	if err != nil {
	...
	_, err = db.AcceptCandidateForDelivery(ctx, store.CandidateRevision{RunID: agentClaim.RunID, LeaseToken: agentClaim.LeaseToken, CodexSessionID: "codex-session", CommitSHA: "accepted", StructuredOutput: []byte(`{"summary":"candidate","checks":[{"command":"go test","outcome":"passed"}]}`), Now: claimedAt, Publication: store.CandidatePublication{Repository: snapshot.Repository, Branch: "ticket-1", ExpectRemoteAbsent: true, Title: "ticket"}}, time.Minute)
	if err != nil {
		t.Fatal(err)
	}
	var isolated []string
	...
	if len(isolated) != 0 {
		t.Fatalf("unreserved delivery was isolated: %#v", isolated)
	}
	projection, err := db.PlanProjectionAt(ctx, version.ID, now)
	if err != nil || projection.Tickets[0].State != "Waiting Review" {
		t.Fatalf("unreserved delivery recovery = %#v, %v", projection, err)
	}
}
