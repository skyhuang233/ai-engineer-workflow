...
func TestBootstrapSkillDeterminesOwnerAndReleaseBeforePlatformPlanning(t *testing.T) {
	...
	content := string(raw)
	for _, required := range []string{"present its owner as the candidate together with its repository name", "With no `origin`, explicitly ask", "repository name and private/public visibility", "before any Platform mutation", "confirmed owner, repository name, visibility, publication state, and domain layout", "-Owner <owner> -RepositoryName <name> -Visibility <private|public> -PublicationState <published|unpublished>", "workflow setup plan --workflow-home $confirmedWorkflowHome --repo (Get-Location).Path --repository-name <confirmed-name> --visibility <private|public> --publication-state <published|unpublished> --domain-layout <single-context|multi-context>", "workflow setup verify --workflow-home $confirmedWorkflowHome --repo (Get-Location).Path", "plan target.workflow_home exactly equals `$confirmedWorkflowHome`", "scripts/resolve-platform-release.ps1", "HostFactsPath = $hostFactsPath", "$resolvedRelease.manifest_path", "$resolvedRelease.host_facts_path", "fresh host may either use the default latest stable release", "On a fresh host, add Version alone", "AllowUpgrade only when an installed lower version", "AllowUpgrade = $true", "verified backup pin automatically supplies exact repair authority", "omit `Version` for that exact pin repair", "both verified pins are missing while the Workflow CLI exists", "confirm the exact installed version", "$releaseArguments.Version = <confirmed-exact-installed-version>", "never use latest-stable selection", "setup inspect-platform-installation", "before the first effect", "contract-validated, forward-only dry run"} {
