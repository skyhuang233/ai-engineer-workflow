...
func TestOpenReadOnlyRejectsOldSchemaWithoutMigrationOrBackup(t *testing.T) {
	...
	if !errors.Is(err, ErrSchemaUpgradeRequired) || !strings.Contains(err.Error(), "schema 59") || !strings.Contains(err.Error(), "schema 64") {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			setup: func(t *testing.T, path string) {
				...
				defer raw.Close()
				if _, err := raw.Exec(`INSERT INTO schema_migrations(version,applied_at) VALUES(65,'future')`); err != nil {
					t.Fatal(err)
				}
...
func TestOpenForLauncherMaintenanceAllowsCompatibleOlderLedgerAndFailsClosedOtherwise(t *testing.T) {
	...
	t.Run("older ledger with stable contract", func(t *testing.T) {
		path := newCurrent(t)
		mutate(t, path, `DELETE FROM schema_migrations WHERE version = 64`)
		...
		defer raw.Close()
		var version int
		if err := raw.QueryRow(`SELECT MAX(version) FROM schema_migrations`).Scan(&version); err != nil || version != 63 {
	...
	for _, test := range []struct {
		...
		{name: "future ledger", statement: `INSERT INTO schema_migrations(version, applied_at) VALUES(65, 'future')`},
