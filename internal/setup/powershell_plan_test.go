...
func TestPowerShellCanonicalPlatformPlanParsesAndAppliesThroughGo(t *testing.T) {
	...
	result, err := (&Engine{Adapter: adapter, ExpectedResultVerifier: passingExpectedResultVerifier, PlatformPreconditionVerifier: passingPlatformPreconditionVerifier}).Apply(t.Context(), canonical, digest)
